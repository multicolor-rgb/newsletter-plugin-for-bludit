...
<label class="mb-2">Sender's User Name:</label>
        ...
        value="<?php echo $this->getValue('sendername');?>" class="sendername form-control">

		<label class="mb-2">Sender's Email Address:</label>
		<input type="email" value="<?php echo $this->getValue('sender');?>"
         name="sender" class="form-control">

		<label class="mb-2">SMTP User Password:</label>
		<input type="password" class="form-control" name="passwordsmtp"
         value="<?php echo $this->getValue('passwordsmtp');?>">


		<label class="mb-2">SMTP Server Address:</label>
		<input type="text" name="smtpserver" 
        value="<?php echo $this->getValue('smtpserver');?>" class="form-control">


		<label>SMTP Port:</label>
		<input type="text" name="port" value="<?php echo $this->getValue('port');?>">

		<label class="mb-2">Requires Authorization?</label>
		<select class="form-select" name="auth">
			<option <?php if($this->getValue('auth')== 'true'){echo 'selected';};?> value="true">Yes</option>
			<option  <?php if($this->getValue('auth')== 'false'){echo 'selected';};?>  value="false">No</option>
		</select>


		<label>Uses SSL?</label>
		<select class="form-select" name="ssl">
			<option  <?php if($this->getValue('ssl')== 'true'){echo 'selected';};?> value="true">Yes</option>
			<option  <?php if($this->getValue('ssl')== 'false'){echo 'selected';};?> value="false">No</option>
		</select>
...
<label class="mb-2">Success send message info</label>
		...
		value="<?php echo $this->getValue('errormsg');?>">
